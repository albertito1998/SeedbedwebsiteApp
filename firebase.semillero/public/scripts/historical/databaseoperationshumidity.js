...
let query = valueSensors.where('timestamp', '>=', startfulldate).where('timestamp', '<=', endfulldate).get()
  .then(snapshot => {
   ...
   var daymax=0;
    ...
    snapshot.forEach(doc => {
      total_count ++;
      suma =suma + parseInt(doc.data().humidityRef);
  ...
  pushData(temperatureChartConfig.data.datasets[0].data, doc.get('humidityRef'))
  console.log(suma)
  console.log(  total_count )
if(tempmax <  doc.data().humidityRef){
  tempmax= doc.data().humidityRef;
  daymax=time;
}
