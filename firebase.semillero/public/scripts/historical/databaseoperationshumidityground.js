...
let query = valueSensors.where('timestamp', '>=', startfulldate).where('timestamp', '<=', endfulldate).get()
  .then(snapshot => {
   ...
   var daymax=0;
    ...
    snapshot.forEach(doc => {
      total_count ++;
      suma =suma+ parseInt(doc.data().humidityground);
  ...
  pushData(temperatureChartConfig.data.datasets[0].data, doc.get('humidityground'))
  console.log( suma )
  console.log(  total_count )
if(tempmax <  doc.data().humidityground){
  tempmax= doc.data().humidityground;
  daymax=time;
}
